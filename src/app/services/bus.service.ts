...
export class BusService extends BaseApiService<any> {
  ...
  getListBus(key: string): Observable<any> {
    return this.http
      .get(`api/Bus?keyword=${key}`)
      .pipe(map((res: ResponeModel) => res.Payload));
  }

  getBusDetail(id: string): Observable<any> {
    return this.http.get(`api/Bus/${id}`).pipe(
      map((res: ResponeModel) => {
        return res.Payload;
      })
    );
  }
  ...
  addNewCar(data: { Name: string }): Observable<any> {
    return this.http.post('api/Bus', data);
  }

  getListDriver(key: string): Observable<any> {
    return this.http
      .get(`api/Driver?keyword=${key}`)
      .pipe(map((res: ResponeModel) => res.Payload));
  }

  getListSupervisor(key: string): Observable<any> {
    return this.http
      .get(`api/Supervisor?keyword=${key}`)
      .pipe(map((res: ResponeModel) => res.Payload));
  }
